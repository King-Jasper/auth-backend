...
public class FundWithdrawalUseCaseImpl implements FundWithdrawalUseCase {
    ...
    public String withdrawalSavingsV2(AuthenticatedUser authenticatedUser, SavingsWithdrawalRequest withdrawalRequest) {
        ...
        SavingsGoalTypeConstant goalType = savingsGoal.getSavingsGoalType();
        if (goalType == SavingsGoalTypeConstant.EMERGENCY_SAVINGS && amountRequested.compareTo(BigDecimal.ZERO) <= 0) {
        ...
        return processCustomerSavingWithdrawalV2(savingsGoal, appUserEntity, amountRequested);
    }

    @Transactional
    private String processCustomerSavingWithdrawalV2(SavingsGoalEntity savingsGoal, AppUserEntity currentUser, BigDecimal amountRequested) {
        LocalDateTime now = LocalDateTime.now();
        boolean isMatured = computeAvailableAmountUseCase.isMaturedSavingsGoal(savingsGoal);
        if(!isMatured) {
            if(savingsGoal.isLockedSavings()) {
        ...
        createWithdrawalRequestV2(savingsGoal, isMatured, currentUser, amountRequested);
        if(isMatured) {
            return "Request queued successfully. Your account will be funded shortly.";
        }
        return "Request queued successfully. Your account will be funded within the next 2 business days.";
    ...
    private void createWithdrawalRequestV2(SavingsGoalEntity savingsGoal, boolean isMaturedGoal, AppUserEntity currentUser, BigDecimal amountRequested) {
        ...
        if (savingsGoal.getSavingsGoalType().equals(SavingsGoalTypeConstant.EMERGENCY_SAVINGS)) {
            withdrawalAmount = amountRequested;
